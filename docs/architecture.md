...
Actionable wakes include captain-relevant status signals, no-verb signals whose crew is not provably working, authenticated check output such as PR merge polling or an X-mode mention, stale panes whose crew is not provably working whether their status log looks terminal or non-terminal, provably-working stale panes that persist past `FM_STALE_ESCALATE_SECS`, declared external waits that remain paused past their recheck cadence (`FM_PAUSE_RESURFACE_SECS`, or `FM_PAUSE_CAPTAIN_RESURFACE_SECS` when a human decision gates the wait), and heartbeat backstop hits.
...
That count is not a ratchet: positive evidence that the pane itself moved, or that its own run record finished a step, returns it to zero, while elapsed time never does, and [`watcher-continuity.md`](watcher-continuity.md#one-alarm-reset-rule-three-applications) owns that reset rule.
...
That cadence is not uniform: `bin/fm-classify-lib.sh`'s `wait_class` reads the status verb and firstmate's durable backlog hold to separate a wait a human must end from one expected to clear on its own, and answers bounded whenever the evidence cannot be read, so a signal that failed to read is never what quiets a lane.
A captain-gated wait is instead rechecked by the event that can actually end it: resolving its hold through `bin/fm-decision-hold.sh` records a durable recheck request against every lane that hold was blocking, and the watcher or away-mode daemon brings those lanes due at the next poll regardless of remaining cadence.
That request lives for the captain-gated window and no longer, so one routed to work no crew is running expires unread rather than reporting a landed ruling against whatever crew next takes that task id; a lane that loses one falls back to its ordinary cadence, because the class is derived from live state rather than recorded.
Every parked lane due for that recheck in one poll is batched into a single check-in wake, rate-limited home-wide by `FM_PARKED_CHECKIN_SECS` through the restart-surviving `state/.last-parked-checkin` mtime, so parked lanes at independent phases cost one wake per window instead of one cycle each.
A poll carrying a lane whose ruling has landed skips that home-wide defer, because the rate limit exists to stop routine parked lanes ending every cycle and a ruling is neither routine nor a cadence hit, and only the lanes an emitted check-in actually carries have their per-lane cadence stamped and their recheck request cleared.
A parked lane whose declared wait actually stops holding leaves the parked classification and surfaces at once, independent of that cadence.
...
The daemon escalates captain-relevant events, plus a recheck for a declared wait that remains idle past its own cadence or whose gating ruling has landed, as one batched, single-line digest using the canonical `away-supervisor` kind from `bin/fm-operational-input.sh` so firstmate can distinguish it structurally from real messages.
...
That reason is required and is recorded on the Bridge row with `phase=force-cleaned`, so a later reader sees both that the check was skipped and the judgement that authorized skipping it.
The row's `outcome` comes from the landed-work determination itself rather than from the flag: skipping the checks does not decide whether the work landed, was discarded, or cannot be told apart.
...
`bin/fm-project-mode.sh` remains the one registry parser for the mechanical consumers that have no task in hand: fleet sync's `local-only` skip, home seeding's refusal and no-mistakes initialization, and the fork freshness sweep's enumeration of registered projects (`--list`, which is why that sweep holds no knowledge of the registry line format).
