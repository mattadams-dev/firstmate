...
state/               volatile runtime signals; gitignored
  ...
  .context-footprint .rebirth-due .rebirth-armed .rebirth-handoff .session-launcher   session-rebirth records: the last context-footprint reading and the session that took it, the over-threshold marker for that session, an asked-for exit awaiting the session's end, what a successor inherits until its own footprint is verified, and the live launch wrapper proving a relaunch is possible; see docs/session-rebirth.md
