...
- `FORK_FRESHNESS: <owner/repo> status=ignored reason=<text>` - an archived fork, one listed in `config/fork-sweep-ignore`, or a project registered `local-only` in `data/projects.md` that this home has not cloned; no action, it is printed only so a skipped fork is never a silent absence.
  `docs/fork-freshness.md` owns the contract.
- `FORK_FRESHNESS: <owner/repo> status=in-sync ... action=task <id> retired` - that fork's earlier sync episode is over, so the sweep expired the idempotency guard and moved its brief to `data/<id>/brief.retired-<stamp>.md`; no action beyond closing the sync task if it is still open in the backlog.
...
- `RETIRE_MANUAL: <owner/repo> is level again but sync task <id> still holds data/<id>/brief.md` beside an `action=task <id> NOT retired: ...` reading - the fork is fine, but its spent idempotency guard could not be moved aside, so the fork's next episode would report `already queued` over a task that closed long ago.
  Move that brief aside by hand now; the guard is what makes the next real behind reading create a real task.
