...
A record repaired by the one-shot backfill also carries a sibling `endpoint_task_id_provenance=` line describing the live read its binding came from; absence of that key means the binding was recorded at spawn ([`verification/endpoint-binding-migration.md`](verification/endpoint-binding-migration.md)).
