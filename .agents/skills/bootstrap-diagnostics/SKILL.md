...
- `... action=task <id> retired RETIRED=<file> (<reason>)`, or `action=task <id> queued RETIRED=<file> (<reason>)` on a behind reading - the marker guarding that fork was backed by no open task (the backlog reports it done, or has no such task), so the sweep moved the spent brief to `data/<id>/<file>` and, when the fork is behind, raised a fresh sync task for it.
  No action; the line exists so a retirement that should not have happened is discoverable afterwards from the reading that decided it and the kept file.
- `GUARD_UNKNOWN: <owner/repo> is behind and data/<id>/brief.md is in place, but whether that sync task is still open could not be read (<reason>)` beside an `action=task <id> not re-queued: ...` reading - the fork is behind and the sweep created NOTHING this run, because it could not tell an open sync task from one that closed long ago.
  Resolve the named cause (usually a missing or unreadable `tasks-axi` backlog) and check that task by hand in this session: this is the one reading shape where a behind fork raises no new work.
...
- `RETIRE_MANUAL: <owner/repo> ... no open task backs data/<id>/brief.md ... could not be moved aside` beside an `action=task <id> NOT retired: ...` or `NOT created: a spent brief ...` reading - the marker is spent but could not be moved, so every later sweep of that fork would report it queued over a task that closed long ago.
  Move that brief aside by hand now; retiring the spent marker is what lets the next real behind reading create a real task.
