...
A record repaired by `bin/fm-migrate-endpoint-binding.sh` also carries a sibling `endpoint_task_id_provenance=` line describing the live read its binding came from; absence of that key means the binding was recorded at spawn ([`verification/endpoint-binding-migration.md`](verification/endpoint-binding-migration.md)).
...
`FM_WEDGE_ALARM_CHANNEL` overrides the file.
Directives are `off` (a position-independent kill switch that disables every active alert), `auto`/`default`, `osascript` (macOS Notification Center banner), `herdr` (herdr UI notification), and `command:<cmd>`, whose dispatch and summary format are owned by [`wedge-alarm.md`](wedge-alarm.md).
...
FM_BACKLOG_OVERRIDE=     # alternate backlog file the watcher and away-mode daemon read when classifying a declared wait, mainly for tests; defaults to <data dir>/backlog.md
FM_TASKS_AXI_BIN=tasks-axi   # tasks-axi executable that declared-wait classification invokes for that backlog read, mainly for tests
...
FM_CREW_STATE_BIN=bin/fm-crew-state.sh   # test override for the crew-state reader used by working/paused watcher triage and by the watcher's completed-step evidence read (--steps)
...
FM_WATCH_ARM_SUCCESSOR=1   # set to 0 to stop the arm layer arming a successor before it delivers a wake; only for a caller that owns continuity itself
FM_WATCH_SUCCESSOR_OUT_TTL=3600   # seconds an unclaimed state/.watch-successor-output.<pid> file survives before the arm layer treats it as debris
...
FM_PAUSE_RESURFACE_SECS=3600       # seconds before an idle BOUNDED declared external wait - one expected to clear on its own - re-surfaces for a recheck in the watcher or away-mode daemon
FM_PAUSE_CAPTAIN_RESURFACE_SECS=21600   # seconds before an idle CAPTAIN-GATED wait re-surfaces instead; elapsed time cannot end a wait a human holds, so this is a backstop against a forgotten hold rather than how such a wait clears - a ruling routed through bin/fm-decision-hold.sh rechecks the lanes it gated at the next poll, and this same window bounds how long that recheck request stays live, so one recorded against work no crew is running expires unread instead of firing at whatever takes that id next
FM_PARKED_CHECKIN_SECS=            # defaults to FM_PAUSE_RESURFACE_SECS; minimum seconds between the watcher's batched parked check-ins, so N parked lanes cost one wake per window instead of N
FM_WEDGE_DEMAND_INSPECT_COUNT=3    # consecutive provably-working stale escalations on the same unchanged pane before demand-deep-inspection is added; the count returns to zero only on positive evidence about that pane, never on elapsed time (docs/watcher-continuity.md owns which evidence counts)
FM_WATCH_TRIAGE_LOG_MAX_BYTES=262144   # size cap for the watcher's absorbed-wake debug log
FM_SAFE_KILL_WAIT=5   # default seconds bin/fm-safe-kill.sh waits for a signalled target to exit before reporting a signalled-but-still-running stop; --wait overrides it per call
FM_SAFE_KILL_LOG_MAX_BYTES=131072   # size cap for state/.safe-kill.log, the durable record of every termination that helper performed or refused
FM_SAFE_KILL_LOG_KEEP_LINES=500   # newest records kept when that log is capped
...
FM_WEDGE_ALARM_CHANNEL=            # override config/wedge-alarm with wedge-alarm active-alert directives, one per non-empty line; off|auto|osascript|herdr|command:<cmd>; absent = auto (macOS -> an OS notification)
