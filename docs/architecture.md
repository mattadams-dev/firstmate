...
Arming the successor is the arm layer's first act of consuming an actionable wake, before that wake is delivered, so continuity no longer depends on whichever adapter sits above it re-arming afterwards.
An attached cycle that closes on a real wake delivers that wake from the successor's own recorded output rather than reporting an unexplained empty close.
