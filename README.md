...
- **A board instead of a scrollback** - the decisions you owe are generated into a local HTML board from an append-only ledger, one card each with nothing already settled competing for the space, so an ask cannot scroll out of view; everything resolved, landed or merely notable lives on a history page beside it. Neither page is ever hand-edited and both are redrawn only when the ledger content changes. You rule by picking an answer on the card and queueing it, or by annotating the card to say it your way; both come back through the review tool's own conversation panel ([docs/bridge.md](docs/bridge.md)).
...
- [docs/verification/session-identity.md](docs/verification/session-identity.md) - active maintainer verification for the session-lock identity basis, its refusal bias, and the mutation table.
