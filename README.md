...
- **A board instead of a scrollback** - the decisions you owe, the criticals, and one row per task are generated into a local HTML board from an append-only ledger, so an ask cannot scroll out of view; the board is never hand-edited, is redrawn only when the ledger content changes, and takes no input of its own - rulings come back through the review tool's own annotation and conversation panel ([docs/bridge.md](docs/bridge.md)).
