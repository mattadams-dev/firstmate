...
state/               volatile runtime signals; gitignored
  ...
  .context-footprint .rebirth-due .rebirth-armed .rebirth-handoff .session-launcher .rebirth-relauncher-alarmed   session-rebirth records: the last context-footprint reading and the session that took it, the over-threshold marker for that session and the session lock it held, an asked-for exit awaiting the session's end, what a successor inherits until its own footprint is verified, the live launch wrapper proving a relaunch is possible, and the once-per-episode dedupe for the Bridge note that says this home cannot rebirth; see docs/session-rebirth.md
