...
Read that owner, or the producing script's header, when you need an exact path, config file, or state record; do not work from memory of the tree.
...
A `state/` entry whose name begins with a dot is a coordination record owned by the script that writes it - watcher, wake queue, away mode, auto-arm, sub-supervisor, and migration state.
Act on one only through that owner's command; never create, edit, or delete one by hand.
...
2. **Bootstrap** - detect-only checks (tool/version problems, GitHub auth, the worktree-tangle check, harness override, dispatch-profile validation, backlog-backend status) always run, but routine confirmations stay silent by default.
   ...
   Home-local stale Herdr projection cleanup and the seven bootstrap MUTATING sweeps - non-executing legacy PR-check migration, fleet sync, secondmate convergence, secondmate liveness, pending remote handoff retry, X-mode artifact writes, and the weekly fork freshness sweep - run only when this session actually holds the lock from step 1.
...
In that case alone, load `validation-supersession` before that worker aborts or touches the branch.
...
A worker hand-editing, committing, aborting, or restarting during an active validation run duplicates pipeline ownership outside the `validation-supersession` sequence; steer it back to the gate response flow.
...
X mode and registered process-to-event sources each require that same live cycle with no fleet work, so a home with either is never free to end a turn blind because the fleet is empty.
...
On every wake-handling turn, whatever the wake reported, read each cloned project's default branch CI result unconditionally and open a Bridge `critical` item when it is red, because nothing else in this fleet watches a default branch after a merge; a run that has not concluded is neither green nor red, and `docs/ci-backstop.md` owns the reasoning, the revert procedure, and the exposure this closes.
...
- `bootstrap-diagnostics` - load whenever the session-start digest's bootstrap section prints an actionable diagnostic line (`MISSING:`, `MISSING_MANUAL:`, `BACKEND_INVALID:`, `NEEDS_GH_AUTH`, `TANGLE:`, `STARTUP_MEMORY_BUDGET:`, `CREW_DISPATCH: invalid`, `FLEET_SYNC:`, `PR_CHECK_MIGRATION:`, `SECONDMATE_SYNC:`, `SECONDMATE_LIVENESS:`, `SECONDMATE_HANDOFF:`, `NUDGE_SECONDMATES:`, `FORK_FRESHNESS:`, or `FMX:`); silence and `BOOTSTRAP_INFO:` need no load.
...
- `validation-supersession` - load when a captain instruction completely invalidates work inside an active no-mistakes run, before the worker aborts or touches the branch.
