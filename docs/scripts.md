...
| `fm-fork-freshness.sh`   | Read every maintained fork's compare-status against its upstream and turn `behind > 0` into a tracked sync task (docs/fork-freshness.md) |
...
| `fm-session-lock-lib.sh` | Shared session-lock identity: the session-id ownership decision, the lock record's line contract, and harness ancestry and holder liveness (docs/verification/session-identity.md) |
...
| `fm-project-mode.sh`     | Resolve a project's registered delivery posture from `data/projects.md`, one name at a time or `--list` for all, for fleet sync, home seeding, and the fork freshness sweep |
...
| `fm-bridge-render.sh`    | The one fold over that ledger: folded state, lifecycle answers, and the generated board and history page |
...
| `fm-migrate-endpoint-binding.sh` | Repair a pre-binding task record's missing `endpoint_task_id=` from a live backend read only, never from `state/` (docs/verification/endpoint-binding-migration.md) |
